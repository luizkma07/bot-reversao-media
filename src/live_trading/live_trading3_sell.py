...
from corretoras.funcoes_bybit import busca_velas, tem_trade_aberto, saldo_da_conta, quantidade_minima_para_operar, abre_venda
    ...
    try:
        ...
        if df.empty:
            ...
            if estado_de_trade == EstadoDeTrade.VENDIDO:
                ...
                # print('Procurando trades de venda...')
                # verifica se temos uma vela referência que fechou abaixo das EMAS
                if df['fechamento'].iloc[-2] < df[f'EMA_{ema_rapida}'].iloc[-2] and df['fechamento'].iloc[-2] < df[f'EMA_{ema_lenta}'].iloc[-2]:
                    ...
                    if df['minima'].iloc[-1] < df['minima'].iloc[-2]:
                        ...
                        preco_stop = df['maxima'].iloc[-qtd_velas_stop : -1].max()
