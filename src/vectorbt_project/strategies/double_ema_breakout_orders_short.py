...
# Entrada na perda da mínima da vela referência, saída no preço de stop ou alvo
# ==========================================
@nb.njit
def double_ema_breakout_short_nb(close, high, low, ema1, ema2, q_stop, rr):
    ...
    for i in range(999, len(close)):
        ...
        if not in_trade and cond1 and cond2 and cond3:
            entry_price = low[i - 1]  # Preço de entrada
            ...
            if current_low <= target_value or current_high >= stop_value:
                exits[i] = target_value if current_low <= target_value else stop_value  # Registrar preço de saída
                in_trade = False
...
def gerar_estrategia(df: pd.DataFrame, estrategia: DoubleEmaBreakout):
    ...
    entries, exits, stop_price, target_price = double_ema_breakout_short_nb(
