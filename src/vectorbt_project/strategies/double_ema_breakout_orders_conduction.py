...
# ==========================================
@nb.njit
def double_ema_breakout_conduction_nb(close, high, low, ema1, ema2, q_stop, rr):
    ...
    for i in range(999, len(close)):
        ...
        if not in_trade and cond1 and cond2 and cond3:
            ...
            cond_saida1 = close[i - 1] < ema2[i - 1]
            cond_saida2 = current_low < low[i - 1]
            ...
            # Condições de saída antes do preço de alvo ou stop:
            elif cond_saida1 and cond_saida2:
                exits[i] = low[i - 1]
                in_trade = False
...
def gerar_estrategia(df: pd.DataFrame, estrategia: DoubleEmaBreakout):
    ...
    entries, exits, stop_price, target_price = double_ema_breakout_conduction_nb(
