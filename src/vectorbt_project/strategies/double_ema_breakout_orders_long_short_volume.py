...
# ==========================================
@nb.njit
def double_ema_breakout_long_short_nb(close, high, low, volume, volume_ma, ema1, ema2, q_stop, rr):
    ...
    for i in range(999, len(close)):
        ...
        long_cond3 = volume[i - 1] > volume_ma[i - 1]       # Volume maior que a média para vela referência
        # long_cond3 = volume[i] > volume_ma[i]             # Volume maior que a média para a vela que ativa a ordem
        # long_cond3 = volume[i-1:i+1].max() > volume_ma[i] # Maior volume das últimas 2 velas maior que a média para a vela que ativa a ordem
        long_cond4 = high[i] > high[i - 1]

        # Condições para short
        short_cond1 = close[i - 1] < ema1[i - 1]
        short_cond2 = close[i - 1] < ema2[i - 1]
        short_cond3 = volume[i - 1] > volume_ma[i - 1]
        short_cond4 = low[i] < low[i - 1]
...
def gerar_estrategia(df: pd.DataFrame, estrategia: DoubleEmaBreakout):
    ...
    volume_ma = pd.Series(volume).rolling(window=20).mean().values # Média móvel de 20 períodos

    # Chamar a função com Numba
    entries, exits, stop_price, target_price, size = double_ema_breakout_long_short_nb(
        close, high, low, volume, volume_ma, ema1, ema2, q_stop, rr
    )
