...
# ==========================================
@nb.njit
def double_ema_breakout_long_short_nb(close, high, low, rsi_1d, rsi_sma_1d, ema1, ema2, q_stop, rr):
    ...
    for i in range(999, len(close)):
        # Condições para long
        long_cond1 = rsi_1d[i] > rsi_sma_1d[i]
        long_cond2 = close[i - 1] > ema1[i - 1]
        long_cond3 = close[i - 1] > ema2[i - 1]
        long_cond4 = high[i] > high[i - 1]

        # Condições para short
        short_cond1 = rsi_1d[i] < rsi_sma_1d[i]
        short_cond2 = close[i - 1] < ema1[i - 1]
        short_cond3 = close[i - 1] < ema2[i - 1]
...
def gerar_estrategia(df: pd.DataFrame, estrategia: DoubleEmaBreakout):
    ...
    rsi_1d = df['rsi_1d'].values
    rsi_sma_1d = df['rsi_sma_1d'].values
    ...
    entries, exits, stop_price, target_price, size = double_ema_breakout_long_short_nb(
        close, high, low, rsi_1d, rsi_sma_1d, ema1, ema2, q_stop, rr
    )
